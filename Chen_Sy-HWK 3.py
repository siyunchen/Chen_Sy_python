...
'''
import pandas as pd #import pandas to use functions from pandas
                    #using pd ro represent pandas in the following functions
pd.read_table('/Users/SiyunChen/Desktop/Chen_Sy_python/iris.data.txt',
              header = 0, sep = ',') 
              #read the data from local file, the header is the text that 
              #the names we add to the columns, and each column is seperated
              #by ',', so we use header = 0 and sep = ','.
              #then each column has a name that we have added. there are total
...
#this single function can give the simple location statistics (Count, Mean, 
#STD, Min, 25%, 50%, 75%, MAX) of numeric variables
...
'''
data.hist() #using data.hist() to plot a a histogram for each of the 
            #numeric columns: sepal length in cm, sepal width in cm,
            #petal length in cm, petal width in cm

data.class0.value_counts().plot(kind='bar')
#plot a bar chart for the nomial column: class
#there are three kinds in class: Iris-setosa, Iris-versicolor and 
#Iris-virginica 
